  ...
  def __init__(self,
    ...
    for seq, s in zip(self.sequences, self._db_sequences):
        ...
        if self.use_prev_mask == False:
            ...
            MySequence = SequenceClip_simple(seq, starting_frame)
            num_frames = MySequence._numframes
            num_clips = int(num_frames / self._length_clip) # Round down
            
            if num_clips==0: 
                continue
            else:
                self.sequence_clips.append(MySequence)
    ...
    for annot, s in zip(self.annotations, self._db_sequences):
        ...
        starting_frame_idx = 0
        starting_frame = int(osp.splitext(osp.basename(images[starting_frame_idx]))[0])
        MySequence = AnnotationClip_simple(annot, starting_frame)
        num_frames = MySequence._numframes
        num_clips = int(num_frames / self._length_clip) # Round down
        ...
        if num_clips==0: 
            ...
            self.annotation_clips.append(MySequence)
