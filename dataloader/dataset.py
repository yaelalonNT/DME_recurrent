...
class MyDataset(data.Dataset):
        ...
        if self.video_mode:
            if self.split == 'train' or self.split == 'val' or self.split == 'trainval':
                ...
                if self.dataset == 'youtube':
                    if self.split == 'train':
                        img_root_dir = self.cfg_youtube.PATH.SEQUENCES_TRAIN
                        annot_root_dir = self.cfg_youtube.PATH.ANNOTATIONS_TRAIN
                    elif self.split == 'val':
                        img_root_dir = self.cfg_youtube.PATH.SEQUENCES_VAL
                        annot_root_dir = self.cfg_youtube.PATH.ANNOTATIONS_VAL
                    else:
                        img_root_dir = self.cfg_youtube.PATH.SEQUENCES_TRAINVAL
                        annot_root_dir = self.cfg_youtube.PATH.ANNOTATIONS_TRAINVAL
                else:
                ...
                for ii in range(max_ii):
                    ...
                    img_flip =  np.flip(img.numpy(),axis=2).copy()
                    ...
                    annot_flip = np.flip(annot,axis=1) 
                    target_flip = self.sequence_from_masks(seq_name,annot_flip)
                    ...
                    targets_flip.append(target_flip)                    
                                    
                return imgs, imgs_flip, targets, targets_flip, seq_name, starting_frame

            else:
                edict = self.get_raw_sample_clip(index)
                img = edict['images']
                if self.dataset == 'youtube':
                    img_root_dir = self.cfg_youtube.PATH.SEQUENCES_TEST
                else:
                    img_root_dir = cfg.PATH.SEQUENCES
    ...
    def sequence_from_masks(self, seq_name, annot):
        ...
        if self.dataset == 'youtube':
            if self.split == 'train':
                json_data = open(self.cfg_youtube.FILES.DB_INFO_TRAIN)
            elif self.split == 'val':
                json_data = open(self.cfg_youtube.FILES.DB_INFO_VAL)
            else:
                json_data = open(self.cfg_youtube.FILES.DB_INFO_TRAINVAL)
